    ...
    private function innerStoreAndExecute(
        ...
        try {
            ...
            if ($explicitTransitionName) {
                $transitions[$explicitMachineName] = $this->machine->getBaseMachine($explicitMachineName)
        ...
        if ($execute == self::STATE_TRANSITION) {
            foreach ($newStates as $name => $newState) {
                ...
                if ($transition) {
                    ...
                    !($state == \FKSDB\Models\Transitions\Machine\Machine::STATE_INIT
                        && $newState == \FKSDB\Models\Transitions\Machine\Machine::STATE_TERMINATED)
                ) {
