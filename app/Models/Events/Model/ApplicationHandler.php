    ...
    final public function onlyExecute(Holder $holder, string $explicitTransitionName): void {
        ...
        try {
            ...
            if (!$transition->matches($holder->getPrimaryHolder()->getModelState())) {
                throw new UnavailableTransitionException($transition, $holder->getPrimaryHolder()->getModel2(true));
            }
            ...
            if ($transition->isCreating()) {
                $this->logger->log(new Message(sprintf(_('Application "%s" created.'), (string)$holder->getPrimaryHolder()->getModel2(true)), Logger::SUCCESS));
            } elseif ($transition->isTerminating()) {
                $this->logger->log(new Message(_('Application deleted.'), Logger::SUCCESS));
            } elseif (isset($transition)) {
                $this->logger->log(new Message(sprintf(_('State of application "%s" changed.'), (string)$holder->getPrimaryHolder()->getModel2(true)), Logger::INFO));
            }
    ...
    private function innerStoreAndExecute(Holder $holder, ?ArrayHash $data, ?Form $form, ?string $explicitTransitionName, ?string $execute): void {
        ...
        try {
            ...
            if ($data && (!isset($transitions[$explicitMachineName]) || !$transitions[$explicitMachineName]->isTerminating())) {
                $this->logger->log(new Message(sprintf(_('Application "%s" saved.'), (string)$holder->getPrimaryHolder()->getModel2(true)), Logger::SUCCESS));
            }
        } catch (ModelDataConflictException $exception) {
            $container = $exception->getReferencedId()->getReferencedContainer();
            $container->setConflicts($exception->getConflicts());
