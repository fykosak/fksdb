...
class BrojureResultsModel extends AbstractResultsModel
{
    ...
    protected function composeQuery(ModelCategory $category): string
    {
        ...
        foreach ($this->getSeries() as $series) {
            $select[] = 'round(SUM(IF(t.series = ' . $series . ', ' . $sum . ", null))) 
            AS '" . self::DATA_PREFIX . $i . "'";
            $i += 1;
        }
        ...
        $select[] = "round(100 * SUM($sum) * " . $studentPilnySumLimitInversed . ")
         AS '" . self::ALIAS_TOTAL_PERCENTAGE . "'";
...
left join submit s ON s.task_id = t.task_id AND s.ct_id = ct.ct_id';
        ...
        $dataAlias = 'data';
        return "select $dataAlias.*, @rownum := @rownum + 1, 
        @rank := IF($dataAlias." . self::ALIAS_SUM . " = @prevSum or ($dataAlias." . self::ALIAS_SUM . ' is null and 
        @prevSum is null), @rank, @rownum) AS `' . self::DATA_RANK_FROM . "`, 
