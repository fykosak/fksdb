...
class MultiResourceAvailability extends AbstractAdjustment {
     ...
     */
    public function __construct($fields, $paramCapacity, $message, Context $database, $includeStates = \FKSDB\Model\Transitions\Machine\Machine::STATE_ANY, $excludeStates = ['cancelled']) {
    ...
    protected function innerAdjust(Form $form, Machine $machine, Holder $holder): void {
        ...
        foreach ($services as $serviceData) {
            ...
            if ($this->includeStates !== \FKSDB\Model\Transitions\Machine\Machine::STATE_ANY) {
                $table->where(BaseHolder::STATE_COLUMN, $this->includeStates);
            }
            if ($this->excludeStates !== \FKSDB\Model\Transitions\Machine\Machine::STATE_ANY) {
