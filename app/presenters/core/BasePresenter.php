...
abstract class BasePresenter extends Presenter implements IJavaScriptCollector, IStylesheetCollector, IAutocompleteJSONProvider, INavigablePresenter {
    ...
    /**
     * @var string ModelRole::ORG|ModelRole::Contestant
     */
    protected $role = null;
    ...
    public function createComponentNav() {
        $control = new \FKSDB\Components\Controls\Navs\Nav($this->yearCalculator, $this->seriesCalculator, $this->session, $this->serviceContest, $this->getTranslator());
        $control->setChoosers($this->getChoosers());
        return $control;
    }
    ...
    protected final function syncRedirect() {
        /**
         * @var $nav \FKSDB\Components\Controls\Navs\Nav
         */
        $nav = $this['nav'];
        $params = [];
        foreach ($this->getChoosers() as $param) {
            if ($param === 'dispatch') {
                if (isset($this->contestId) && isset($this->role)) {
                    $params['contestId'] = $this->contestId;
                    $params['role'] = $this->role;
                } else {
                    throw new BadRequestException('Nedovolené použite dispatchChooseru, nejaký parameter chýba:' . $param);
                }
                continue;
            }
            ...
            if (property_exists($this, $param)) {
                ...
                throw new BadRequestException('Nedovolené použite Chooseru, nejaký parameter chýba:' . $param);
            }

        }
        $newParams = $nav->init((object)$params);
        ...
        foreach ($this->getChoosers() as $param) {
            if ($param === 'dispatch') {
                $params['contestId'] = $newParams->contestId ?: $this->contestId;
                continue;
            }
            $redirectParams[$param] = $newParams->{$param} ?: $this->{$param};
        }

        if (!isset($this->role) || $this->role === $newParams->role) {
            $this->redirect('this', $redirectParams);
        } else {
            if ($newParams->role === ModelRole::ORG) {
                $this->redirect(':Org:Dashboard', $redirectParams);
            } else {
                $this->redirect(':Public:Dashboard', $redirectParams);
            }
        }
    }
