...
function FileState(props: OwnProps & DispatchProps & StateProps) {
    const translator = useContext(TranslatorContext);
    const {submit, onDeleteFile, actions} = props;
        ...
        <button aria-hidden="true" className="pull-right btn btn-outline-warning"
                ...
                onClick={() => {
                    if (window.confirm(translator.getText('Remove submit?'))) {
                        onDeleteFile(actions.getAction('revoke'));
                    }
                }}>&times;</button>
        <div className="text-center p-2">
            <a href={props.actions.getAction('download')}>
