    ...
    public function handleCheck(): void {
        ...
        foreach ($this->seriesTable->getSubmits() as $submit) {
            if ($submit->source === ModelSubmit::SOURCE_UPLOAD && !$this->uploadedStorage->fileExists($submit)) {
                $errors++;
                $this->flashMessage(sprintf(_('Uploaded submit #%d is broken'), $submit->submit_id), MessageLevel::ERROR->value);
            }

            if ($submit->corrected && !$this->correctedStorage->fileExists($submit)) {
                $errors++;
                $this->flashMessage(sprintf(_('Corrected submit #%d is broken'), $submit->submit_id), MessageLevel::ERROR->value);
            }
            if (!$submit->corrected && $this->correctedStorage->fileExists($submit)) {
                $errors++;
                $this->flashMessage(sprintf(_('Uploaded unregister corrected submit #%d'), $submit->submit_id), MessageLevel::ERROR->value);
            }
        }
        $this->flashMessage(sprintf(_('Test done, found %d errors'), $errors), $errors ?MessageLevel::WARNING->value : MessageLevel::SUCCESS->value);
