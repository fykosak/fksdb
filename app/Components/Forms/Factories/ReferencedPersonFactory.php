...
class ReferencedPersonFactory extends Object implements IReferencedSetter {
     ...
     * @param "person" | "person_info" | "person_history" | string $sub
     * @param $field
     * @param $acYear
     * @param $options
     * @return bool|mixed|\MPostContact|null
     */
    public static function getPersonValue(ModelPerson $person = null, $sub, $field, $acYear, $options) {
        Debugger::barDump($field);
        Debugger::barDump($sub);
        if (!$person) {
            return null;
        }
        switch ($sub) {
            ...
            case 'person_accommodation':
                return ($person->ref(\DbNames::TAB_EVENT_PERSON_ACCOMMODATION));
            default:
