...
class ReferencedPersonContainer extends ReferencedContainer {
                    ...
                    $control->addCondition(function () { // we use this workaround not to call getValue inside validation out of transaction
                        ...
                        ->addRule(function (BaseControl $control) use ($fullFieldName) {
                            ...
                            if ($foundPerson && $foundPerson->getPrimary() != $personId) {
                                $this->getReferencedId()->setValue($foundPerson, self::MODE_FORCE);
                                return false;
                            }
                            return true;
     ...
     * @throws JsonException
     */
    public function setModel(IModel $model = null, string $mode = self::MODE_NORMAL) {

        $modifiable = $model ? $this->modifiabilityResolver->isModifiable($model) : true;
        $resolution = $model ? $this->visibilityResolver->getResolutionMode($model) : ReferencedPersonHandler::RESOLUTION_OVERWRITE;
    ...
    protected function getPersonValue($person, string $sub, string $field, $options) {
        ...
        switch ($sub) {
            ...
            case 'person_info':
                ...
                return $result;
            case 'person_history':
                return ($history = $person->getHistory($this->acYear, (bool)($options & self::EXTRAPOLATE))) ? $history[$field] : null;
            case 'post_contact_d':
