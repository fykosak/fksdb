...
class EventBilletedPerson extends BaseGrid {
    ...
    protected function configure($presenter) {
        ...
        $this->addButton('confirmPaymentAll', _('Confirm all payment'))
            ...
            ->setLink(function ($row) {
                return $this->link('confirmPaymentAll!', [$row->person_id, $row->event_person_accommodation->event_id]);
            });

        $this->addButton('deletePaymentAll', _('Delete all payment'))->setText(_('Delete all payment'))
            ->setClass('btn btn-sm btn-warning')
            ->setLink(function ($row) {
                return $this->link('deletePaymentAll!', [$row->person_id, $row->event_person_accommodation->event_id]);
            });
    ...
    public function handleConfirmPaymentAll($personId, $eventId) {
        $rows = $this->serviceEventPersonAccommodation->getAllAccommodationForPersonAndEvent($personId, $eventId);
    ...
    public function handleDeletePaymentAll($personId, $eventId) {
        $rows = $this->serviceEventPersonAccommodation->getAllAccommodationForPersonAndEvent($personId, $eventId);
        foreach ($rows as $row) {
            $model = \ModelEventPersonAccommodation::createFromTableRow($row);
            $model->update(['status' => \ModelEventPersonAccommodation::STATUS_WAITING_FOR_PAYMENT]);
