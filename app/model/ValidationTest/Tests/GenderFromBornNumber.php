...
class GenderFromBornNumber extends ValidationTest {
    ...
    public function run(ModelPerson $person): ValidationLog {
        ...
        if (!$person->gender) {
            return new ValidationLog($this->getTitle(), _('Gender is not set'), self::LVL_WARNING);
        }
        if (!$info->born_id) {
            return new ValidationLog($this->getTitle(), _('Born number is not set'), self::LVL_INFO);
        }

        if (BornNumber::getGender($info->born_id) != $person->gender) {
            return new ValidationLog($this->getTitle(), 'Gender not match born number', self::LVL_DANGER);
        } else {
            return new ValidationLog($this->getTitle(), 'Gender match born number', self::LVL_SUCCESS);
        }
    }
