...
use FKSDB\Payment\Transition\Transitions\Fyziklani14Payment;
...
class MachineFactory {
    ...
    private function createTransitionsGenerator(ModelEvent $event): AbstractTransitionsGenerator {
        ...
        if (($event->event_type_id === 1) && ($event->event_year === 14)) {
            return new Fyziklani14Payment($this->servicePayment,
                $this->connection,
                $this->transitionsFactory,
                $this->symbolGeneratorFactory,
                $this->priceCalculatorFactory,
