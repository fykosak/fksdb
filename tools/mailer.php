...
foreach ($query as $model) {
    ...
    try {
        ...
        $mailer->send($message);
        $serviceEmailMessage->updateModel2($model, ['state' => ModelEmailMessage::STATE_SENT, 'sent' => new DateTime()]);
    } catch (Exception $e) {
        $serviceEmailMessage->updateModel2($model, ['state' => ModelEmailMessage::STATE_FAILED]);
