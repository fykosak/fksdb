...
class HandlerCreateTest extends HandlerTestCase
{
    ...
    public function testCapacityNotOk(): void
    {
        ...
        $this->itemService->explorer->getConnection()->onQuery[] = function ($connection, ResultSet $resultSet) {
            var_dump($resultSet->getQueryString());
            var_dump($resultSet->getRowCount());
        };
        /* Assert::exception(
    ...
    public function testCapacityTransaction(): void
    {
        ...
        $this->groupService->explorer->getConnection()->transaction(function () use ($person2) {
